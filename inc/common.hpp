...
#define SAFE_DESTROY(func, resource, ...) do {\
	...
	#define ThrowIfFailed(result, ...) ::democollection::_ThrowIfFailed(result, #result, __FILE__, __LINE__, ##__VA_ARGS__)

	void _ThrowIfFalse(bool statement, const char* lineText, const char* filename, int lineNumber, const char* details = nullptr);
	#define ThrowIfFalse(statement, ...) ::democollection::_ThrowIfFalse(statement, #statement, __FILE__, __LINE__, ##__VA_ARGS__)
